...
def parse_with_ai(text: str):
    if not text.strip():
        return [], [], ""
    prompt = f"Extract recipe JSON {{ingredients: [], instructions: [], notes: \"\"}} from:\n\n{text[:14000]}"
    try:
        resp = client.chat.completions.create(model="gpt-4o-mini", messages=[{"role": "user", "content": prompt}], temperature=0.1, max_tokens=800)
...
async def extract_recipe(request: ExtractRequest):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            ings, insts, notes = parse_with_ai(text)
            return {
                "title": info.get("title", "Video Recipe"),
                "ingredients": ings,
                "instructions": insts,
                "thumbnail": info.get("thumbnail", ""),
                "notes": notes or "from video"
            }
    except Exception as e:
        raise HTTPException(400, detail=f"Video failed: {str(e)}")
    finally:
