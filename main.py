# SHAWN MAC COOKIE FINAL VICTORY - NOV 9 2025 - AUTO CLEAR CACHE - YOUR COOKIES LOADED
import os
import re
import json
import requests
...
def parse_with_ai(text: str):
    ...
    prompt = f"Extract recipe JSON {{ingredients: [], instructions: [], notes: ''}} from: {text[:15000]}"
    try:
...
async def extract_recipe(request: ExtractRequest):
    ...
    try:
        ...
        return {"title": data.get("title"), "ingredients": data.get("ingredients", []), "instructions": (data.get("instructions") or "").split("\n"), "image": data.get("image", ""), "yield": data.get("yields", ""), "time": data.get("total_time", 0), "notes": "Scraped"}
    except: pass
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            return {"title": info.get('title', 'Video'), "ingredients": ings, "instructions": inst, "image": info.get('thumbnail', ''), "time": info.get('duration', 0), "notes": f"MAC COOKIES VICTORY NOV 9 • {notes}"}
...
@app.get("/")
async def root():
    return {"message": "SHAWN MAC COOKIE FINAL VICTORY - NOV 9 2025 - COOKIES + CORS + IG WORKING - BOLT PROMPT SUCCESS"}
