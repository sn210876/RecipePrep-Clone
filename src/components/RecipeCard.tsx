...
export function RecipeCard({
  ...
  const loadSocialPost = async () => {
    try {
      let postData = null;
      const { data: postByRecipeId } = await supabase
        .from('posts')
        .select('*')
  ...
  return (
    <>
      {/* MAIN CARD - Now uses flex + h-full to prevent cutoff */}
      <Card
        className="group flex flex-col h-full overflow-hidden hover:shadow-xl transition-all duration-300 border-0 bg-white cursor-pointer rounded-xl shadow-sm"
        onClick={() => setShowDetail(true)}
      >
        {/* IMAGE CONTAINER - Fixed aspect ratio + proper positioning */}
        <div className="relative overflow-hidden aspect-video bg-gray-100 rounded-t-xl">
          <img
            src={
              recipe.imageUrl?.includes('instagram.com') || recipe.imageUrl?.includes('cdninstagram.com')
                ? `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/image-proxy?url=${encodeURIComponent(recipe.imageUrl.replace(/&amp;/g, '&'))}`
                : recipe.imageUrl || '/placeholder-food.jpg'
            }
        ...
        <CardContent className="p-5 space-y-4 flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Badge variant="outline" className={`text-xs ${difficultyColors[recipe.difficulty]}`}>
                {recipe.difficulty}
              </Badge>
            </div>
          </div>
        ...
        {/* FOOTER - Always at bottom, never cut off */}
        <CardFooter className="p-5 pt-0 mt-auto flex flex-col gap-2">
          ...
          {showReviewButton && (
            ...
            >
              <span className="text-base mr-2">Fire</span>
              {reviewCount > 0 ? (
          ...
          {showReviewButton && socialPost && (
            ...
            >
              <span className="text-base mr-2">Chat</span>
              <span className="flex items-center gap-2">
                See Reviews
      ...
      <RecipeDetailModal recipe={recipe} open={showDetail} onOpenChange={setShowDetail} />
      <ReviewForm
        recipe={recipe}
        open={showReviewForm}
        onOpenChange={setShowReviewForm}
        onReviewSubmitted={loadReviewData}
      />
    </>
  );
}