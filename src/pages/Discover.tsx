...
export function Discover({ onNavigateToMessages, onNavigate: _onNavigate, sharedPostId, onPostViewed }: DiscoverProps = {}) {
  ...
  useEffect(() => {
    const handleSharedPost = (e: any) => {
      ...
      if (postId) setCommentModalPostId(postId);
    };
    ...
    const match = window.location.pathname.match(/^\/post\/([a-f0-9-]{36})$/);
    if (match) {
      setCommentModalPostId(match[1]);
      window.history.replaceState({}, '', '/discover');
    }
    return () => window.removeEventListener('open-shared-post', handleSharedPost);
  }, []);
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      ...
      if (data) {
        ...
        const mapped = data.map(n => ({ ...n, actor: profilesMap.get(n.actor_id) || { username: 'Unknown', avatar_url: null } }));
        setNotifications(mapped);
        setUnreadNotifications(mapped.filter(n => !n.read).length);
      }
    };
    loadNotifications();
    const channel = supabase.channel('notifications-changes').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `user_id=eq.${currentUserId}` }, loadNotifications).subscribe();
  ...
  useEffect(() => {
    if (sharedPostId && currentUserId) {
      const loadSharedPost = async () => {
        const { data: post } = await supabase.from('posts').select('*, profiles(username, avatar_url), likes(user_id), comments(id)').eq('id', sharedPostId).maybeSingle();
        if (post) {
  ...
  const fetchPosts = useCallback(async (pageNum: number, isRefresh = false) => {
    // ... your existing fetchPosts logic (unchanged) ...
    // (keeping it exactly as you had it — no changes needed here)
    try {
      let query = supabase.from('posts').select('*').order('created_at', { ascending: false });
      if (filterHashtag) {
        const { data: hashtagData } = await supabase.from('hashtags').select('id').eq('tag', filterHashtag.toLowerCase()).maybeSingle();
        if (hashtagData) {
          const { data: postHashtags } = await supabase.from('post_hashtags').select('post_id').eq('hashtag_id', hashtagData.id);
          const postIds = (postHashtags || []).map(ph => ph.post_id);
          if (postIds.length === 0) { setPosts([]); setHasMore(false); setLoading(false); return; }
          query = query.in('id', postIds);
        } else { setPosts([]); setHasMore(false); setLoading(false); return; }
      }
      const { data: postsData, error: postsError } = await query.range(pageNum * POSTS_PER_PAGE, (pageNum + 1) * POSTS_PER_PAGE - 1);
      ...
      const postsWithDetails = await Promise.all((postsData || []).map(async (post) => {
        ...
        const { data: comments } = await supabase.from('comments').select('id, text, created_at, user_id, rating').eq('post_id', post.id).order('created_at', { ascending: false }).limit(2);
        const commentsWithProfiles = await Promise.all((comments || []).map(async (comment) => {
          const { data: commentProfile } = await supabase.from('profiles').select('username').eq('id', comment.user_id).single();
          return { ...comment, profiles: commentProfile };
        }));
        const { data: postRatingsData } = await supabase.from('post_ratings').select('rating').eq('post_id', post.id);
        const ratingsCount = postRatingsData?.length || 0;
        const averageRating = ratingsCount > 0 ? postRatingsData!.reduce((sum, r) => sum + r.rating, 0) / ratingsCount : 0;
        setPostRatings(prev => ({ ...prev, [post.id]: { average: averageRating, count: ratingsCount } }));
        return { ...post, profiles: profile, likes: likes || [], comments: commentsWithProfiles, _count: { likes: likes?.length || 0, comments: commentsCount || 0 } };
      }));
      if (isRefresh) { setPosts(postsWithDetails); setPage(0); } else { setPosts(prev => [...prev, ...postsWithDetails]); }
  ...
  useEffect(() => {
    let isMounted = true;
    const loadInitialPosts = async () => { if (isMounted) await fetchPosts(0, true); };
    loadInitialPosts();
    const channel = supabase.channel('posts-and-comments-changes')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'posts' }, async (payload) => {
        if (!isMounted) return;
        const newPost = payload.new as any;
        const { data: profile } = await supabase.from('profiles').select('username, avatar_url').eq('id', newPost.user_id).maybeSingle();
        const fullPost = { ...newPost, profiles: profile || { username: 'User', avatar_url: null }, likes: [], comments: [], _count: { likes: 0, comments: 0 } };
        setPosts(prev => prev.some(p => p.id === fullPost.id) ? prev : [fullPost, ...prev]);
      })
      .subscribe();
    return () => { isMounted = false; if (channel) supabase.removeChannel(channel); };
  }, [fetchPosts]);

  const handleLoadMore = () => { const nextPage = page + 1; setPage(nextPage); fetchPosts(nextPage); };

  // ... all your other handlers (toggleFollow, handleDeletePost, etc.) remain 100% unchanged ...
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="max-w-sm mx-auto" onClick={() => { setShowNotifications(false); setShowSearchResults(false); }}>
        {/* Header, Search, Notifications — unchanged */}
        ...
        <div className="pt-20">
          {filterHashtag && (
            <div className="bg-blue-50 border-b border-blue-200 p-3 flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setFilterHashtag(null)} className="text-blue-600 hover:text-blue-700">Clear</Button>
            </div>
          )}

          {viewingUserId ? (
            <UserProfileView userId={viewingUserId} currentUserId={currentUserId} posts={posts} isFollowing={followingUsers.has(viewingUserId)} onBack={() => setViewingUserId(null)} onToggleFollow={toggleFollow} onMessage={onNavigateToMessages} />
          ) : posts.length === 0 ? (
            <div className="text-center py-12"><p className="text-gray-500">No posts yet. Be the first to share!</p></div>
          ) : (
            <>
              {posts.map(post => {
                ...
                return (
                  <div key={post.id} className="flex flex-col min-h-0">
                    <div className="bg-white border-b border-gray-200 mb-2 flex flex-col">
                      {/* Entire post card now uses flex — NO MORE CUTOFF */}
                      {/* ... all your existing post JSX (header, image, likes, caption, etc.) ... */}
                      {/* (Exactly the same as before, just wrapped properly) */}

                      <div className="px-4 py-3 flex items-center justify-between">
                        {/* User avatar + username */}
                        <div className="flex items-center gap-3">
                          {post.profiles?.avatar_url ? (
                            <img src={post.profiles.avatar_url} alt={post.profiles.username} className="w-8 h-8 rounded-full object-cover" />
                          ) : (
                          ...
                          <div className="flex items-center gap-1">
                            <button onClick={() => window.location.href = `/${post.profiles?.username}`} className="font-semibold text-sm hover:underline">
                              {post.profiles?.username}
                            </button>
                            {post.user_id === '51ad04fa-6d63-4c45-9423-76183eea7b39' && <Crown className="w-4 h-4 text-yellow-500 fill-yellow-500" />}
                          </div>
                        </div>
                        {(isOwnPost || isAdmin) && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => setEditingPost({ id: post.id, caption: post.caption || '', recipeUrl: post.recipe_url || '', photoUrl: post.photo_url || '' })} className="cursor-pointer">
                                <Edit3 className="w-4 h-4 mr-2" /> Edit post
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => setDeletePostId(post.id)} className="cursor-pointer text-red-600">
                      ...
                      <div className="relative">
                        {post.image_url ? (
                          <img src={post.image_url} alt={post.title || 'Post'} className="w-full aspect-square object-cover" />
                        ) : post.video_url ? (
                          <video src={post.video_url} controls className="w-full aspect-square object-cover" />
                        ) : null}
                          ...
                          <div className="absolute top-4 right-4 z-10">
                            <button
                              onClick={(e) => {
                                ...
                                document.querySelectorAll('audio').forEach(a => { if (a.id !== `audio-${post.id}`) a.pause(); });
                                if (audio.paused) { audio.play(); if (btn) btn.textContent = 'Pause'; }
                                else { audio.pause(); if (btn) btn.textContent = 'Play'; }
                              }}
                            ...
                            <div className="flex items-end justify-between gap-2">
                              ...
                              {postRatings[post.id]?.count > 0 && (
                                <div className="flex items-center gap-1 bg-black/50 px-2 py-1 rounded-full">
                                  ...
                                  <span className="text-white text-xs font-semibold">{postRatings[post.id].average.toFixed(1)}</span>
                                  <span className="text-white/70 text-xs">({postRatings[post.id].count})</span>
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <div className="px-4 py-3 space-y-2">
                        <div className="flex items-center gap-4">
                          <button onClick={() => toggleLike(post.id)} className="transition-transform hover:scale-110 relative">
                            ...
                            {post._count?.likes > 0 && <span className="absolute -top-1 -right-1 bg-yellow-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{post._count.likes}</span>}
                          </button>
                          <button onClick={() => setCommentModalPostId(post.id)} className="transition-transform hover:scale-110 relative">
                            ...
                            {post._count?.comments > 0 && <span className="absolute -top-1 -right-1 bg-yellow-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center">{post._count.comments}</span>}
                          </button>
                        ...
                        {post.caption && (
                          <div className="text-sm">
                            <span className="inline-flex items-center gap-1">
                              ...
                              {post.user_id === '51ad04fa-6d63-4c45-9423-76183eea7b39' && <Crown className="w-3 h-3 text-yellow-500 fill-yellow-500" />}
                            </span>{' '}
                            <span className="text-gray-700">
                              {makeHashtagsClickable(post.caption, (tag) => { setFilterHashtag(tag); setViewingUserId(null); })}
                            </span>
                          </div>
                        )}
        ...
        {/* Modals */}
        {commentModalPostId && <CommentModal postId={commentModalPostId} isOpen={!!commentModalPostId} onClose={() => setCommentModalPostId(null)} onCommentPosted={() => fetchPosts(0, true)} />}
        ...
        {selectedRecipe && <RecipeDetailModal recipe={selectedRecipe} open={!!selectedRecipe} onOpenChange={(open) => !open && setSelectedRecipe(null)} />}
