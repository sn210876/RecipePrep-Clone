...
export function Discover({ onNavigateToMessages, onNavigate: _onNavigate, sharedPostId, onPostViewed }: DiscoverProps = {}) {
  ...
  // OPTIMIZED: Single query with proper joins instead of multiple queries
  const fetchPosts = useCallback(async (pageNum: number, isRefresh = false) => {
    try {
      ...
      // OPTIMIZED: Fetch posts with profile in one query
      let query = supabase
        .from('posts')
        .select(`
          *,
      ...
      // OPTIMIZED: Batch fetch all likes and comments in 2 queries instead of N queries
      ...
      const postsWithDetails = postsData.map(post => {
        ...
        return {
          ...post,
          likes,
          comments: comments.slice(0, 2), // Only keep 2 latest
          _count: {
            likes: likes.length,
