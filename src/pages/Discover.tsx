...
export function Discover({ onNavigateToMessages, onNavigate, sharedPostId, onPostViewed }: DiscoverProps = {}) {
  ...
  //added this manually 
  // ——————————————————————————————————————
// ADD THIS FUNCTION ANYWHERE IN Discover.tsx (near the top with other handlers)
// ——————————————————————————————————————
const sendNotification = async (type: 'like' | 'comment', postId?: string) => {
  if (!currentUserId) return;

  // Find the post owner (for like/comment)
  if (type !== 'like' && type !== 'comment') return;
  const post = posts.find(p => p.id === postId);
  if (!post || post.user_id === currentUserId) return; // don't notify yourself

  try {
    await supabase.from('notifications').insert({
      user_id: post.user_id,      // who gets the notification
      actor_id: currentUserId,    // who did the action
...
// ——————————————————————————————————————
// REPLACE YOUR ENTIRE toggleLike FUNCTION WITH THIS ONE
// ——————————————————————————————————————
